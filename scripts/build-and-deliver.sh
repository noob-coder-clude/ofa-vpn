#!/usr/bin/env bash
#
# build-and-deliver.sh
# اسکریپت کامل بیلد OFA VPN روی سرور لینوکس
# 
# همه‌چی رو خودش انجام می‌ده: SDK، NDK، native libs، GEO، بیلد APK
# آخرش APK رو کپی می‌کنه به یه مسیر قابل دانلود
#
# اجرا:  bash build-and-deliver.sh
#
set -euo pipefail

# ── رنگ‌ها ──
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
RED='\033[0;31m'
NC='\033[0m'

info()  { echo -e "${GREEN}✅ $1${NC}"; }
warn()  { echo -e "${YELLOW}⚠️  $1${NC}"; }
err()   { echo -e "${RED}❌ $1${NC}"; }

# ── مسیرها و نسخه‌ها ──
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
SDK="/opt/android-sdk"
NDK_VER="r25c"
NDK="${SDK}/ndk/${NDK_VER}"
XRAY_VER="v25.7.23"
T2S_VER="v2.8.3"
OUTPUT_DIR="/tmp/ofa-output"

export ANDROID_HOME="${SDK}"
export ANDROID_SDK_ROOT="${SDK}"
export JAVA_HOME="/usr/lib/jvm/java-17-openjdk-amd64"
export PATH="${SDK}/cmdline-tools/latest/bin:${SDK}/platform-tools:${NDK}:${JAVA_HOME}/bin:${PATH}"

# ── ۱. نصب پیش‌نیازها ──
info "نصب پیش‌نیازهای سیستم ..."
apt-get update -qq
apt-get install -y -qq openjdk-17-jdk wget unzip curl git python3 >/dev/null 2>&1

# ── ۲. Android SDK + NDK ──
if [ ! -d "${SDK}/cmdline-tools/latest" ]; then
    info "دانلود Android SDK command-line tools ..."
    mkdir -p "${SDK}/cmdline-tools"
    cd /tmp
    wget -q "https://dl.google.com/android/repository/commandlinetools-linux-11076708_latest.zip" -O cmdtools.zip
    unzip -o cmdtools.zip -d "${SDK}/cmdline-tools" >/dev/null
    mv "${SDK}/cmdline-tools/cmdline-tools" "${SDK}/cmdline-tools/latest" 2>/dev/null || true
    rm -f cmdtools.zip
fi

info "نصب SDK packages (platform-tools, android-34, build-tools, NDK) ..."
yes | sdkmanager --sdk_root="${SDK}" --licenses >/dev/null 2>&1
sdkmanager --sdk_root="${SDK}" \
    "platform-tools" \
    "platforms;android-34" \
    "build-tools;34.0.0" \
    "ndk;${NDK_VER}" >/dev/null 2>&1

# ── ۳. local.properties ──
info "نوشتن local.properties ..."
cat > "${ROOT}/local.properties" <<EOF
sdk.dir=${SDK}
ndk.dir=${NDK}
EOF

# ── ۴. دانلود native libs ──
JNILIBS="${ROOT}/app/src/main/jniLibs/arm64-v8a"
ASSETS="${ROOT}/app/src/main/assets"
mkdir -p "${JNILIBS}" "${ASSETS}"

if [ ! -f "${JNILIBS}/libxray.so" ]; then
    info "دانلود libxray.so (${XRAY_VER}) ..."
    cd /tmp
    wget -q "https://github.com/XTLS/Xray-core/releases/download/${XRAY_VER}/Xray-android-arm64-v8a.zip" -O xray.zip
    unzip -o xray.zip -d /tmp/xray-ext >/dev/null
    cp /tmp/xray-ext/xray "${JNILIBS}/libxray.so"
    chmod +x "${JNILIBS}/libxray.so"
    rm -rf /tmp/xray-ext xray.zip
    info "libxray.so آماده شد"
fi

if [ ! -f "${JNILIBS}/libtun2socks.so" ]; then
    info "دانلود libtun2socks.so (${T2S_VER}) ..."
    cd /tmp
    if wget -q "https://github.com/xjasonlyu/tun2socks/releases/download/${T2S_VER}/tun2socks-android-arm64.zip" -O t2s.zip; then
        unzip -o t2s.zip -d /tmp/t2s-ext >/dev/null
        BIN=$(find /tmp/t2s-ext -type f -name "tun2socks*" ! -name "*.zip" | head -1)
        cp "${BIN}" "${JNILIBS}/libtun2socks.so"
        chmod +x "${JNILIBS}/libtun2socks.so"
        rm -rf /tmp/t2s-ext t2s.zip
        info "libtun2socks.so آماده شد"
    else
        warn "دانلود tun2socks ناموفق — بدونش هم کار می‌کنه"
        rm -f t2s.zip
    fi
fi

# ── ۵. دانلود GEO assets ──
if [ ! -f "${ASSETS}/geoip.dat" ]; then
    info "دانلود geoip.dat ..."
    curl -fsSL "https://github.com/XTLS/Xray-core/releases/download/${XRAY_VER}/geoip.dat" -o "${ASSETS}/geoip.dat"
fi
if [ ! -f "${ASSETS}/geosite.dat" ]; then
    info "دانلود geosite.dat ..."
    curl -fsSL "https://github.com/XTLS/Xray-core/releases/download/${XRAY_VER}/geosite.dat" -o "${ASSETS}/geosite.dat"
fi
info "GEO assets آماده شد"

# ── ۶. Gradle wrapper (اگه نبود) ──
if [ ! -f "${ROOT}/gradle/wrapper/gradle-wrapper.jar" ]; then
    info "ساخت gradle-wrapper.jar ..."
    cd "${ROOT}"
    if command -v gradle &>/dev/null; then
        gradle wrapper --gradle-version 8.9 --distribution-type bin >/dev/null 2>&1
    else
        # نصب gradle موقت
        wget -q "https://services.gradle.org/distributions/gradle-8.9-bin.zip" -O /tmp/gradle.zip
        unzip -q /tmp/gradle.zip -d /opt
        /opt/gradle-8.9/bin/gradle wrapper --gradle-version 8.9 --distribution-type bin >/dev/null 2>&1
        rm -f /tmp/gradle.zip
    fi
fi

# ── ۷. بیلد APK ──
info "شروع بیلد APK ..."
cd "${ROOT}"
chmod +x gradlew
./gradlew assembleDebug --no-daemon --stacktrace

# ── ۸. کپی خروجی ──
APK_SRC="${ROOT}/app/build/outputs/apk/debug/app-debug.apk"
if [ -f "${APK_SRC}" ]; then
    mkdir -p "${OUTPUT_DIR}"
    cp "${APK_SRC}" "${OUTPUT_DIR}/ofa-vpn-debug.apk"
    APK_SIZE=$(du -h "${OUTPUT_DIR}/ofa-vpn-debug.apk" | cut -f1)
    info "APK ساخته شد!"
    info "مسیر: ${OUTPUT_DIR}/ofa-vpn-debug.apk"
    info "حجم: ${APK_SIZE}"
    echo ""
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "📦  برای دانلود APK:"
    echo "    scp <user>@<server>:${OUTPUT_DIR}/ofa-vpn-debug.apk ."
    echo ""
    echo "یا اگه python server داری:"
    echo "    cd ${OUTPUT_DIR} && python3 -m http.server 8080"
    echo "    # بعد از مرورگر: http://<server-ip>:8080/ofa-vpn-debug.apk"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
else
    err "APK ساخته نشه! logها رو ببین"
    exit 1
fi
